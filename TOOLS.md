...
| **Bright Data AI Lead Generator** | Open-source Python + Streamlit app. Scrapes B2B contact/company data via Bright Data's scraping API, qualifies/scores leads against an ideal-customer-profile using OpenAI + LangChain (FAISS embeddings, LLMChain). Requires `BRIGHT_DATA_API_TOKEN`, `OPENAI_API_KEY`. | **Adopted -- phase 1 (this integration).** Python-native, matches the existing stack (Docker, n8n, script_writer.py already use OpenAI). Broader than LinkedIn -- general web/company data. Scaffolded below. |
